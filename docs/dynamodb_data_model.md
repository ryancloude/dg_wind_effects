...
Silver candidate loading uses this GSI and falls back to table scan if needed.

## Operational Queries (AWS CLI)

### Get one Silver checkpoint
```powershell
aws dynamodb get-item `
  --table-name $env:PDGA_DDB_TABLE `
  --region $env:AWS_REGION `
  --key '{"pk":{"S":"PIPELINE#SILVER_LIVE_RESULTS"},"sk":{"S":"EVENT#90008"}}'