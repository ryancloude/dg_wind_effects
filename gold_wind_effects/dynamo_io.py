...
from gold_wind_effects.models import GOLD_CHECKPOINT_PK, PIPELINE_NAME


@dataclass(frozen=True)
class GoldEventCandidate:
    event_id: int
    event_year: int
    round_s3_key: str
    hole_s3_key: str
...
def load_gold_event_candidates(
    *,
    table_name: str,
    aws_region: Optional[str],
    event_ids: list[int] | None = None,
) -> list[GoldEventCandidate]:
    table = _ddb_resource(aws_region).Table(table_name)

    enriched_items = _query_pipeline_items(table=table, pk="PIPELINE#SILVER_WEATHER_ENRICHED")

    enriched_success: dict[int, dict[str, Any]] = {}
    for item in enriched_items:
        ...
        if event_id is None:
            continue
        round_key = str(item.get("round_s3_key", "")).strip()
        hole_key = str(item.get("hole_s3_key", "")).strip()
        if not round_key or not hole_key:
            continue
        enriched_success[int(event_id)] = item

    candidate_ids = sorted(enriched_success.keys())
    if event_ids:
        allowed = {int(x) for x in event_ids}
        candidate_ids = [eid for eid in candidate_ids if eid in allowed]

    out: list[GoldEventCandidate] = []
    for event_id in candidate_ids:
        src = enriched_success[event_id]
        ...
        out.append(
            GoldEventCandidate(
                ...
                round_s3_key=str(src.get("round_s3_key", "")).strip(),
...
def load_gold_event_checkpoints(
    *,
    table_name: str,
    aws_region: Optional[str],
) -> dict[int, dict[str, Any]]:
    table = _ddb_resource(aws_region).Table(table_name)
    items = _query_pipeline_items(table=table, pk=GOLD_CHECKPOINT_PK)
...
def get_gold_event_checkpoint(
    *,
    ...
    table = _ddb_resource(aws_region).Table(table_name)
    return _get_item(table, pk=GOLD_CHECKPOINT_PK, sk=f"EVENT#{int(event_id)}")


def put_gold_event_checkpoint(
    *,
    table_name: str,
    event_id: int,
    run_id: str,
