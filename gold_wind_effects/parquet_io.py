...
def build_round_output_key(*, event_year: int, event_id: int) -> str:
    return (
        "gold/pdga/wind_effects/player_rounds_features/"
        f"event_year={int(event_year)}/"
        f"tourn_id={int(event_id)}/"
        "player_rounds_features.parquet"
    )


def build_hole_output_key(*, event_year: int, event_id: int) -> str:
    return (
        "gold/pdga/wind_effects/player_holes_features/"
        f"event_year={int(event_year)}/"
        f"tourn_id={int(event_id)}/"
        "player_holes_features.parquet"
    )


def build_quarantine_key(*, event_year: int, event_id: int, run_id: str) -> str:
    return (
        "gold/pdga/wind_effects/quarantine/"
        f"event_year={int(event_year)}/"
        f"tourn_id={int(event_id)}/"
        f"run_id={run_id}/"
        "dq_errors.json"
...
def overwrite_event_tables(
    *,
    ...
    s3.put_object(Bucket=bucket, Key=round_key, Body=_to_parquet_bytes(round_rows), ContentType="application/octet-stream")
    s3.put_object(Bucket=bucket, Key=hole_key, Body=_to_parquet_bytes(hole_rows), ContentType="application/octet-stream")
