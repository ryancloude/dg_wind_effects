...
from train_round_wind_model.models import MODEL_INPUTS_ROUND_PREFIX
...
def list_model_input_round_objects(
    *,
    bucket: str,
    prefix: str = MODEL_INPUTS_ROUND_PREFIX,
    ...
    while True:
        ...
        for obj in resp.get("Contents", []):
            ...
            if allowed is not None:
                ...
                if not match:
                    continue
                tourn_id = int(match.group(1))
                if tourn_id not in allowed:
                    continue
...
def load_parquet_table(*, bucket: str, key: str, s3_client=None) -> pa.Table:
    s3 = _s3_client(s3_client)
    body = s3.get_object(Bucket=bucket, Key=key)["Body"].read()
    return pq.read_table(BytesIO(body))


def load_model_input_round_dataframe(
    *,
    bucket: str,
    prefix: str = MODEL_INPUTS_ROUND_PREFIX,
    event_ids: list[int] | None = None,
    ...
    objects = list_model_input_round_objects(
        bucket=bucket,
        prefix=prefix,
        event_ids=event_ids,
        s3_client=s3_client,
    ...
    for obj in objects:
        tables.append(load_parquet_table(bucket=bucket, key=obj["key"], s3_client=s3_client))
