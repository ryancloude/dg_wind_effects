...
from train_round_wind_model.models import PIPELINE_NAME, TRAINING_CHECKPOINT_PK
...
def _to_dynamodb_safe(value: Any) -> Any:
    ...
    if isinstance(value, set):
        return [_to_dynamodb_safe(v) for v in sorted(value, key=lambda x: str(x))]
...
def get_training_checkpoint(
    *,
    ...
    resp = table.get_item(
        Key={
            "pk": TRAINING_CHECKPOINT_PK,
            "sk": f"TRAINING#{training_request_fingerprint}",
        },
